...
- **모든 구현·수정은 각 PC의 centbloom 원본 저장소의 로컬 `main`에서 진행한다.** 집 PC는 `C:\Users\Siyun\Desktop\works\centbloom`, 회사 PC는 `C:\Users\proje\Desktop\개인용\centbloom`을 사용한다. PC를 바꾸면 GitHub 최신 main과 로컬 변경을 먼저 확인·통합한다. 시작할 때 실제 저장소 루트·브랜치를 확인하며 별도 작업 브랜치·Git worktree를 만들지 않는다. 사용자 확인 주소는 이 루트에서 실행하는 `http://localhost:3000`이다.
