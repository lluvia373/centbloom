...
- 코드 작업은 항상 별도 로컬 브랜치와 작업 폴더(Git worktree)를 만들어 진행한다. main에서 직접 구현하지 않는다. 푸시·병합 지시가 오면 검증 후 반영하고 로컬 main을 원격 main과 맞춘다. 병합을 확인한 작업 브랜치는 실행 프로세스를 종료하고 폴더까지 삭제한다. 보존할 비공개 자료는 삭제 전에 Git 제외 영역으로 옮긴다.
...
6. 문서를 신설·이름 변경·역할 변경하면 이 AGENTS의 기준표, README 안내, 관련 참조를 함께 갱신한다. 문서 변경 완료 보고 전에 `npm run check:docs`를 통과해야 한다. 이 검사는 관리 목록·상대 링크·Markdown 제목 앵커·사업 문서 Git 추적 여부를 확인하며 `npm run lint`에도 포함된다. 역할 중복·결정 모순·명령의 의미는 직접 검토한다. 버전·설정 값은 package.json·package-lock.json·wrangler.jsonc가 기준이다.
