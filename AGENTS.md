...
- **모든 구현·수정은 `C:\Users\Siyun\Desktop\works\centifolio`의 로컬 `main`에서 진행한다.** 시작할 때 실제 저장소 루트·브랜치를 확인하며 별도 작업 브랜치·Git worktree를 만들지 않는다. 사용자 확인 주소는 이 루트에서 실행하는 `http://localhost:3000`이다.
