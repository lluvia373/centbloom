...
전략 조사: 2026-09-06 · 코드 대조: 2026-09-12(main 0983225) · 제품 결정 대조: 2026-09-17(D019). 검색 정책·공개 응답·검색 실적을 오늘 재조사한 문서는 아니다. 구현·공개 배포는 별도 승인 범위를 따른다.

**현재 적용: [DECISIONS.md](./DECISIONS.md) D005·D008에 따라 첫 공개 버전의 회원 토론은 제외한다.** 공개 시장·종목·읽을거리에서 관심 저장·개인 투자 기록으로 연결한다. 아래 포럼/회원 게시물 운영 제안은 후속 검토에만 적용한다. D009의 캘린더·발표 상세는 실제 출처/수치가 있는 공개 진입점으로 검토하며, D010은 오른쪽 광고 사용과 추가 광고 도입 방향이며 추가 위치·모바일 형식은 미정이다. 광고 배치는 검색 성과를 대신하지 않는다. D012에 따라 샘플 체험을 전환 경로/성과로 세지 않고 로그인→실제 관심 저장·거래·노트 작성을 측정한다. D011 구루 포트폴리오는 미구현 계획이므로 현재 유입 실적·첫 출시 필수 조건에 넣지 않는다. D016의 계정 동기화는 재방문 가설에 연결한다. 관심종목은 이번 로컬 개발의 서버 연결 범위이며 노트 동기화는 미구현이다. 원격 적용·다중 기기 검증·공개 배포를 별도로 확인한다. D017의 가격 등락 색 통일도 검색/전환 실적을 뜻하지 않는다.

D019에 따라 검색 유입과 공개 탐색은 포트폴리오 관리·관심 저장·내 종목 후속 정보로 연결한다. 공개 뉴스 조회량만을 제품 가치의 달성으로 보지 않으며 실제 개인 기능 이용과 재방문은 별도로 검증한다. 세부 지표·목표치는 미정이고 새 분석 수집 착수 승인을 뜻하지 않는다. [제품 방향](./PRODUCT_SPEC.md#제품-방향)을 따른다.
...
09-13 홈 일정의 다가오는 발표/최근 결과 개편은 기존 공개 /calendar·발표 상세 경로를 유지한다. 검색 노출·재방문 개선은 가설이며 로컬 UI 적용을 유입 실적으로 세지 않는다.

## 현재 확인된 장애

D018은 사용자가 다른 투자자와 구별되는 사람이라고 느끼는 차별화·자기 이미지를 목표로 한다. 정보 가치 향상과는 별개의 제품 경험 가설이며 시각적 분위기·말투·반복 이용으로 설계한다. 평소 대비 변화 비교는 홈→종목→관심 저장·재방문 가설에 연결한다. ‘보고받는 느낌’은 내부 설계 비유로만 쓰고 제품 문구에는 드러내지 않는다. 기존 관심 별표를 통한 조사·후속 변화 우선 배치는 로컬 개발 범위이며 체감이나 재방문 성과는 미검증이다. [계산 범위](./PRODUCT_SPEC.md#평소와-다른-움직임)를 따른다. 09-12 로컬 구현은 종목별 최근 자료를 조사한 카드 묶음을 초기 HTML에도 포함한다. 직전 5거래일 비교와 종목 뉴스 제목·매체·원문 링크를 바로 보여주고 카드는 3개씩 넘긴다. 후속 페이지 카드를 별도 색인 가능 URL로 간주하지 않으며 독자적인 원인 분석 기사로 표현하지 않는다. 검색 수집·색인 개선이나 유입 성과는 미검증이며 공개 배포 전이다. 개인화 화면은 계정의 관심목록으로 브라우저에서 구성하며 공개 HTML·공유 캐시에 사용자 선택을 넣지 않는다. 개인별 정렬이나 종목 자료 API를 별도 검색 유입 페이지로 세지 않는다. 향후 이용 분석에서는 변화 카드 진입·종목 이동·실제 서버 저장·다중 기기 재접속·재방문을 구분한다. 승인된 회원의 마지막 방문 이후 변화 연결은 미구현이며, 이에 필요한 방문 기준 기록과 일반 클릭/체류 추적은 구분한다. 사용자 숨김 기능은 도입하지 않는다. 뉴스 약 0.5초는 체감 속도 희망 목표로 관리하고 새 출시 필수 기준으로 추가하지 않는다. 기존 출시 조건·기한을 유지한다.
