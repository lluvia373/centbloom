...
**현재 적용: [DECISIONS.md](./DECISIONS.md) D005·D008에 따라 첫 공개 버전의 회원 토론은 제외한다.** 공개 시장·종목·읽을거리에서 관심 저장·개인 투자 기록으로 연결한다. 아래 포럼/회원 게시물 운영 제안은 후속 검토에만 적용한다. D009의 캘린더·발표 상세는 실제 출처/수치가 있는 공개 진입점으로 검토하며, D010 광고 배치는 검색 성과를 대신하지 않는다. D012에 따라 샘플 체험을 전환 경로/성과로 세지 않고 로그인→실제 관심 저장·거래·노트 작성을 측정한다. D011 구루 포트폴리오는 미구현 계획이므로 현재 유입 실적·첫 출시 필수 조건에 넣지 않는다.
