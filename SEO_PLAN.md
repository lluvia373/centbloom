...
**현재 적용: [DECISIONS.md](./DECISIONS.md) D005·D008에 따라 첫 공개 버전의 회원 토론은 제외한다.** 공개 시장·종목·읽을거리에서 관심 저장·개인 투자 기록으로 연결한다. 아래 포럼/회원 게시물 운영 제안은 후속 검토에만 적용한다. D009의 캘린더·발표 상세는 실제 출처/수치가 있는 공개 진입점으로 검토하며, D010 광고 배치는 검색 성과를 대신하지 않는다. D011 구루 포트폴리오는 미구현 계획이므로 현재 유입 실적·첫 출시 필수 조건에 넣지 않는다.
...
아래는 09-06 이전 HEAD `735cc42`와 당시 공개 URL의 진단이다. 이후 공개 홈·종목·읽을거리 분리가 배포되었다는 기록은 [PROJECT_STATUS.md](./PROJECT_STATUS.md)에 있다. 아래 로그인 제한·리디렉션을 현재 장애로 재사용하지 않는다. 검색 수집·사이트맵·색인 상태는 현재 배포 기준 재확인이 필요하다.
...
| 우선순위 | 할 일 | 완료 기준 |
| --- | --- | --- |
| P0 | 공개 열람 분리 | 홈·종목 상세·공개 읽을거리는 비로그인 열람. 개인 포트폴리오·설정·노트는 인증과 서버 권한 유지 |
