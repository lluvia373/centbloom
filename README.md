...
기본 작업 위치는 각 PC의 centbloom 원본 저장소의 로컬 `main`이다. 사용자가 해당 작업에 브랜치를 명시하면 같은 원본의 로컬 브랜치를 사용한다. 시작·수정·최종 보고 전에 실제 경로와 브랜치가 요청에 맞는지 확인하고 보고 첫 줄에 작업 브랜치를 표시한다. 명령·불일치/충돌 처리 기준은 [AGENTS 작업 원칙](./AGENTS.md#작업응답-원칙)을 따른다. 사용자의 `커밋`, `푸시`, `배포` 지시는 같은 승인으로 보며, 하나를 지시하면 의도한 변경 커밋 → GitHub 푸시 → Cloudflare 자동 배포·공개 응답 확인까지 완료한다. 사용자가 `커밋만`처럼 범위를 제한하면 그 지시를 우선한다. 승인된 병합을 마치면 같은 작업에서 완료 브랜치와 전용 작업 폴더까지 정리한다. GitHub 자동 삭제는 원격 브랜치에 적용되며 로컬 정리·자료 보존 조건도 위 작업 원칙을 따른다.
...
**개발 중에는 변경 부분만 빠르게 검사하고, 전체 검사는 푸시 직전에 모아서 실행한다.** 검사 대상 선정·재실행 조건·실제 연결 확인·보조 AI 중복 방지는 [AGENTS 검사 원칙](./AGENTS.md#작업응답-원칙)을 따른다. 문서·테스트 수정은 구현과 함께 하며 아직 실행하지 않은 전체 검사는 푸시 검증 대기로 기록한다. 파일 저장·개발 서버 시작·커밋에는 검사 훅을 두지 않는다.

각 PC에서 [배포와 같은 Node 버전](./CLOUDFLARE.md#준비와-실행)을 선택하고 Git clone/pull로 최신 main을 받은 뒤 `npm ci`를 실행하면 prepare가 [푸시 훅](./.githooks/pre-push)을 자동 등록한다. ZIP 다운로드에는 Git 정보가 없어 훅을 등록할 수 없다. 현재 체크아웃에서 수동 등록하려면 `npm run hooks:install` 또는 `node tests/install-git-hooks.mjs`를 실행한다. 기존 다른 훅 설정이 있으면 덮어쓰지 않고 안내한다. CI와 Git 없는 배포 소스에서는 등록을 건너뛴다.

| 시점 | 실행 내용 |
| --- | --- |
| 개발 중 | 변경한 동작에 직접 연결된 좁고 빠른 검사. 통과 후 관련 변경이 없으면 반복하지 않음. 문서만 바꾸면 문장·참조·diff 확인. `docs:guide`는 검사 없이 기능 안내만 조회 |
| `git push` 직전 · 코드/설정 포함 또는 범위 불명 | 문서 연결 → 새 디자인 위반 → ESLint → 순환 참조 → 전체 기능/DB 테스트 → Next.js 생산 빌드(타입 검사 포함) |
| `git push` 직전 · 허용된 개발 문서만 변경 | 문서 연결 검사만 실행. 전체 테스트·앱 빌드 생략. 판정 범위와 예외는 [문서 전용 변경](./CLOUDFLARE.md#문서-전용-변경) 참조 |
| 검사 실패 | 푸시 중단. 문제를 수정·커밋하고 다시 푸시 |
| 푸시 이후 | 코드/설정 변경은 Cloudflare 배포 환경 검사·OpenNext 빌드 수행. 허용 문서만 바뀌면 경로 제외 설정으로 자동 배포 생략. 운영 계정·실제 광고·브라우저 사용성 확인은 별도 |

검사 대상과 전송 코드가 같도록 현재 HEAD의 커밋만 검사한다. 커밋하지 않은 추적/미추적 파일이 있거나 다른 커밋을 전송하려 하면 중단한다(Git 제외 파일은 제외). 검사 중 코드가 바뀌어도 중단한다. 원격 참조 삭제나 보낼 변경이 없는 경우에는 검사를 생략한다. 전체 검사 명령 `npm run check:push`도 같은 조건으로 검사만 수행하며 커밋·푸시·배포를 실행하지 않는다. 이 수동 명령은 사용자가 전체 검사만 요청한 경우 등에 한정한다. 일반 푸시에서는 수동 실행하지 않고 pre-push에 맡긴다. 현재 훅은 수동 성공 결과를 재사용하지 않으므로 수동 전체 검사 후 `git push`를 하면 전체가 두 번 실행된다. 이를 피하려고 훅을 건너뛰지는 않는다.
...
개발 중에는 `node --test tests/<변경과 직접 관련된 파일>.test.mjs`처럼 대상을 지정한다. 필요하면 해당 파일 안에서도 `--test-name-pattern`으로 사례를 좁힌다. `npm run lint`, `npm run check:docs`, `npm run check:design`, `node --test tests/*.test.mjs`는 범위가 넓은 명령이므로 개발 중 빠른 확인용으로 사용하지 않는다. 전체 실행은 푸시 단계나 사용자의 명시적 검사 요청에 한정한다. `npm run audit:design`은 기존 미정리까지 포함해 검사하므로 푸시 기본 검사가 아니다. 새 위반 0과 전체 준수를 구분한다. 실제 PostgreSQL 두 연결·브라우저·성능 실측도 자동으로 덧붙이지 않고 AGENTS의 범위·비용 기준에 따라 아래 절차를 선택한다.
