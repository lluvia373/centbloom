...
기본 작업 위치는 각 PC의 centbloom 원본 저장소의 로컬 `main`이다. 사용자가 해당 작업에 브랜치를 명시하면 같은 원본의 로컬 브랜치를 사용한다. 시작·수정·최종 보고 전에 실제 경로와 브랜치가 요청에 맞는지 확인하고 보고 첫 줄에 작업 브랜치를 표시한다. 명령·불일치/충돌 처리 기준은 [AGENTS 작업 원칙](./AGENTS.md#작업응답-원칙)을 따른다. 커밋·푸시·배포는 사용자 지시 범위에서 진행한다. 승인된 병합을 마치면 같은 작업에서 완료 브랜치와 전용 작업 폴더까지 정리한다. GitHub 자동 삭제는 원격 브랜치에 적용되며 로컬 정리·자료 보존 조건도 위 작업 원칙을 따른다.
