...
const inputClass =
  "w-full rounded-xl border border-[#e1e7e2] bg-white px-4 py-3 text-sm text-[#1b2c26] outline-none transition focus:border-[#236b50] focus:ring-2 focus:ring-[#236b50]/10";
...
export function TransactionForm({
  ...
  return (
    <div className="grid items-start gap-6 lg:grid-cols-[1.05fr_1fr]">
      <section className="overflow-hidden rounded-2xl border border-[#e8ece9] bg-white">
        <div className="border-b border-[#edf0ed] p-6 sm:p-7">
          <div className="mb-5 flex items-center gap-3">
            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#edf5ef] text-xs font-semibold text-[#236b50]">
              01
            </span>
            <h2 className="text-base font-semibold text-[#1b2c26]">
              어떤 종목을 거래했나요?
            </h2>
          </div>
          <div className="relative">
            <Search className="absolute left-4 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-[#617365]" />
            <input
              aria-label="종목 검색"
              type="search"
              value={query}
              ...
              className={cn(inputClass, "bg-[#f7f9f7] py-3.5 pl-11 pr-10")}
            />
            {query && (
              <button
                type="button"
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 rounded-md p-1 text-[#617365] hover:bg-[#e8ece9]"
              >
                <X className="h-4 w-4" />
              </button>
            )}
        ...
        <div className="p-4 sm:p-5">
          <div className="mb-3 flex items-center justify-between px-2">
            <h3 className="text-xs font-medium text-[#617365]">
              {query.trim() ? "검색 결과" : "빠르게 찾아보기"}
            </h3>
            <span className="text-[10px] uppercase tracking-wider text-[#65745f]">
              STOCKS & ETFS
            </span>
          </div>
          <div aria-live="polite" className="min-h-56">
            {searchLoading ? (
              <div className="flex items-center justify-center py-20 text-sm text-[#617365]">
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                종목을 찾고 있어요
              </div>
            ) : searchError ? (
              <div className="p-8 text-center">
                <p className="text-sm leading-6 text-[#617365]">
                  {searchError}
                </p>
                <button
                  type="button"
                  onClick={() => setRetry((value) => value + 1)}
                  className="mt-4 text-sm font-medium text-[#236b50]"
                >
                  다시 검색하기
                </button>
              </div>
              ...
              <div className="py-16 text-center">
                <Search className="mx-auto mb-3 h-7 w-7 text-[#65745f]" />
                <p className="text-sm text-[#617365]">
                ...
                >
                  <span
                    className={cn(
                      ...
                      [
                        "bg-[#eceeed] text-[#303a35]",
                        "bg-[#ecf4dc] text-[#527425]",
                        "bg-[#eaf0f8] text-[#35639a]",
                        "bg-[#faf0e9] text-[#8b5d3b]",
                      ][index % 4],
                    )}
                  >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-semibold text-[#1b2c26]">
                      {stock.name}
                    </span>
                    <span className="mt-1 block text-[11px] text-[#617365]">
                      {stock.symbol}{" "}
                      <span className="mx-1.5 text-[#65745f]">·</span>{" "}
                      {stock.exchange}
                    </span>
                  </span>
                  {selected?.symbol === stock.symbol ? (
                    <Check className="h-4 w-4 text-[#236b50]" />
                  ) : (
                    <ArrowRight className="h-4 w-4 text-[#65745f] group-hover:text-[#236b50]" />
                  )}
                </button>
              ))
            )}
        ...
        <div className="flex items-start gap-2 border-t border-[#edf0ed] bg-[#fbfcfa] px-6 py-4 text-xs leading-5 text-[#617365]">
      ...
      <section className="rounded-2xl border border-[#e8ece9] bg-white p-6 sm:p-7">
        ...
        {selected ? (
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="flex items-center justify-between gap-3 rounded-xl bg-[#f7f9f7] p-4">
              <div>
                <p className="font-semibold text-[#1b2c26]">{selected.name}</p>
              ...
              <Link
                ...
                className="flex items-center gap-1 text-xs font-medium text-[#236b50]"
              >
                종목 보기
                <ArrowUpRight className="h-3.5 w-3.5" />
              </Link>
            </div>
            <div className="grid grid-cols-2 gap-2 rounded-xl bg-[#f4f6f3] p-1">
              {(["buy", "sell"] as const).map((type) => (
                <button
                  ...
                  className={cn(
                    ...
                    txType === type
                      ? "bg-white text-[#236b50] shadow-sm"
                      : "text-[#617365] hover:text-[#1b2c26]",
                  )}
                >
                  {type === "buy" ? (
            ...
            {ohlcLoading && (
              <div className="flex items-center justify-center rounded-xl bg-[#f7f9f7] py-8 text-xs text-[#617365]">
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
            ...
            {ohlcError && (
              <div
                role="alert"
                className="rounded-xl bg-[#fcf2ef] p-4 text-xs leading-6 text-[#ab4e42]"
              >
                <p>{ohlcError}</p>
            ...
            {ohlc && !ohlcLoading && (
              <>
                <PriceRangePicker
                  ...
                  onChange={setPrice}
                />
                <p className="text-[11px] text-[#617365]">
            ...
            {totalAmount > 0 && ohlc && (
              <div className="flex items-center justify-between border-t border-[#edf0ed] pt-4 text-sm">
                <span className="text-[#617365]">
                  {txType === "buy" ? "총 매수 금액" : "총 매도 금액"}{" "}
                  <span className="text-[10px]">(수수료 반영)</span>
                </span>
            ...
            {error && (
              <p
                role="alert"
                className="rounded-xl bg-[#fcf2ef] px-4 py-3 text-sm text-[#ab4e42]"
              >
                {error}
              </p>
            )}
            {success && (
              <div
                role="status"
                className="flex items-center justify-between gap-2 rounded-xl bg-[#edf5ef] px-4 py-3 text-xs text-[#236b50]"
              >
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#236b50] py-3.5 text-sm font-medium text-white transition hover:bg-[#1b563f] disabled:cursor-not-allowed disabled:opacity-40"
            >
              <Plus className="h-4 w-4" />
              {txType === "buy" ? "매수 기록 저장" : "매도 기록 저장"}
            </button>
            <p className="text-center text-[11px] text-[#617365]">
              입력한 거래는 내 포트폴리오에 저장됩니다.
            </p>
          </form>
        ) : (
          <div className="flex min-h-[415px] flex-col items-center justify-center text-center">
            <div className="relative mb-6 flex h-20 w-20 items-center justify-center rounded-[24px] bg-[#edf5ef]">
              <Plus className="h-8 w-8 text-[#617365]" />
              <span className="absolute -right-2 -top-2 h-5 w-5 rounded-full border-4 border-white bg-[#d9e7ce]" />
            </div>
            <h3 className="text-base font-semibold text-[#1b2c26]">
              나의 투자를 한 줄씩
            </h3>
            <p className="mt-3 max-w-56 text-sm leading-6 text-[#617365]">
              먼저 종목을 선택해 주세요.
              <br />
