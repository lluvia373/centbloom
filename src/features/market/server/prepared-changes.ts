...
export const CHANGES_KEY = "market-changes:v2:us";
export const CHANGES_REFRESH_MS = 5 * 60_000;
export const CHANGES_MAX_AGE_MS = 30 * 60_000;
export interface PreparedChanges { version: 2; preparedAt: number; feed: ResearchedChangesFeed }
...
export function usableChanges(value: unknown, now = Date.now()): PreparedChanges | null {
  ...
  if (row.version !== 2 || !finite(row.preparedAt) || row.preparedAt > now + 300_000
  ...
  const items = row.feed.items.flatMap(item => {
    if (!usableItem(item, now) || seen.has(item.quote.symbol)) return [];
    const story = item.story && Array.isArray(item.story.symbols) ? selectChangeStory(item, [item.story], now) : null;
    seen.add(item.quote.symbol);
    return [{ ...item, story }];
  });
  if (row.feed.items.length && !items.length) return null;
  return { version: 2, preparedAt: row.preparedAt, feed: { ...row.feed, items, expiresAt: row.preparedAt + CHANGES_MAX_AGE_MS } };
}
