...
| 로컬 | `codex/remove-workspace-card`: 사이드바 카드 제거·국가별 시세·검색·종목 마크·새 브랜드 저장 키와 이전 데이터 호환·보유종목 수정/전체 삭제 | 사용자 지시로 로컬 전용. 커밋·푸시·기능 배포하지 않음. 동시 진행 중인 다른 변경은 별도 검증 |
...
운영 스키마·RLS·제약은 읽기 전용 메타데이터로 대조했고 사용자 행은 변경하지 않았다. 증분 마이그레이션·호출 코드·로컬 SQL 검사·[적용/복구 절차](./CLOUDFLARE.md#거래-저장-원자성-업데이트--운영-미적용)를 준비했다. 독립 staging의 다중 연결 advisory lock 경합, 실제 Supabase/PostgREST 응답 유실·JWT 전환·다중 기기, 공개 Worker에서 Yahoo/Worker 동작은 운영 적용 승인 전 남은 절차다. PGlite 단일 연결과 모의 I/O 결과를 이 검증의 대체로 표시하지 않는다. 이미 전량 매도한 과거 종목은 성과 조회에서 제외한다. 장기 거래정지는 필요한 종목의 조회 범위를 거래 시작까지 확장하며, 확장 후에도 가격이 없으면 오류를 표시하고 저장하지 않는다.
...
1. 진행 중 로컬 변경을 검증하고 공개 배포 상태와 구분해 기록.
